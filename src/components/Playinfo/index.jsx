...
export default function Playinfo() {
  ...
  //订阅进度条位置
  unsubscribe("mark");
  const updatemark = useSubscribe("mark", function (msg, data) {
    setMarks(data.mark);
  ...
  const changetime = (e) => {
    ...
    setMarks({
      ...
      [e]:
        (e / 100) *
          (new Date(marks["100"]).valueOf() - new Date(marks["0"]).valueOf()) +
        new Date(marks["0"]).valueOf(),
      100: marks["100"],
    });
    //发布播放位置
    publish("playtime", e);
  ...
  //订阅按下柱状图时的播放信息
  unsubscribe("playb");
  const updateplay = useSubscribe("playb", function (msg, data) {
    setplay(data);
  });
  ...
  const updatePlayinfo = useSubscribe("showplayinfo", function (msg, data) {
  ...
  return (
    ...
    >
      <Button type="primary" className="playinfobtn"></Button>
    </Popover>
  );
}
