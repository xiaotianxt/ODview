...
*/
          ...
          csv(csvoption)
            ...
            .then((jsonObj) => {
              ...
              const SLON =
                columnsnames[
                  columnsnames
                    .map((f) => f.toLowerCase().indexOf("slon") >= 0)
                    .indexOf(true)
                ];
              const SLAT =
                columnsnames[
                  columnsnames
                    .map((f) => f.toLowerCase().indexOf("slat") >= 0)
                    .indexOf(true)
                ];
              const ELON =
                columnsnames[
                  columnsnames
                    .map((f) => f.toLowerCase().indexOf("elon") >= 0)
                    .indexOf(true)
                ];
              const ELAT =
                columnsnames[
                  columnsnames
                    .map((f) => f.toLowerCase().indexOf("elat") >= 0)
                    .indexOf(true)
                ];
              const COUNT =
                columnsnames[
                  columnsnames
                    .map((f) => f.toLowerCase().indexOf("count") >= 0)
                    .indexOf(true)
                ];
              form.setFieldsValue({
  ...
  const processod = (field, data) => {
    ...
    for (let key in locations_tmp) {
      locations.push({
        id: key,
        count: locations_tmp[key],
        lon: parseFloat(key.split(",")[0]),
    ...
    <>
      <Col span={24}>
        <Card
          title="OD流向图"
          extra={
            <Tooltip title="Import OD data to show flow map">
              <InfoCircleOutlined />
            </Tooltip>
          }
          bordered={false}
                ...
                <Row gutters={4}>
                  ...
                  <Col span={24}>
                    <Form.Item label="显示节点" name="locationTotalsEnabled">
                      <Switch
                        size="small"
                        checked={config.locationTotalsEnabled}
                      />
                    </Form.Item>
                  </Col>
                  <Col span={24}>
                    <Form.Item
                      label="黑色模式（地图样式为light时使用）"
                      name="darkMode"
                    >
                ...
                <Row gutters={4}>
                  ...
                  <Col span={24}>
                    <Form.Item label="聚类层数" name="clusteringLevel">
                      <Slider
                        ...
                        step={1}
                        value={
                          typeof config.clusteringLevel === "number"
                  ...
                  <Col span={24}>
                    <Form.Item label="褪色透明" name="fadeOpacityEnabled">
                      <Switch
                        size="small"
                        checked={config.fadeOpacityEnabled}
                      />
                    </Form.Item>
                  </Col>
                  <Col span={24}>
                    <Form.Item label="褪色比例" name="fadeAmount">
                      <Slider
                        ...
                        step={0.1}
                        value={
                          typeof config.fadeAmount === "number"
                            ? config.fadeAmount
                            : 0
                        }
                      />
